...
var LoadingGameOff = cc.Class({
    ...
    handleConfig_3: function handleConfig_3(result) {
        ...
        if (cc.sys.os === cc.sys.OS_IOS && require("GameManager").getInstance().statePoay === true) {
            this.node.getComponent("HotUpdateOff").checkUpdate();
        } else {
            this.node.getComponent("HotUpdateOff").showSlotGame();
        }
        return;
        if (doc.crash != null) {
            cc.NGWlog("vao crash");
