...
cc.Class({
  ...
  loadFreeChip() {
    ...
    cc.NGWlog("GM.pro: ", require("GameManager").getInstance().promotionInfo);
    ...
    cc.NGWlog("this.countMailAg la: " + this.countMailAg);
  ...
  reloadList() {
    ...
    for (var i = 0; i < dataSize; i++) {
      cc.NGWlog('dataSize cua freeChip la 1', dataSize);
      let item;
    ...
    for (var i = 0; i < dataSize; i++) {
      cc.NGWlog('dataSize cua freeChip la 2', dataSize);
      let item;
    ...
    cc.NGWlog(' list_view.content la %d', this.list_view.content.children.length);
  }
});
