...
var GiftView = cc.Class({
    ...
    setInfo() {
        this.onPopOn();
        this.onClickTabGift();
        for (var i = 0; i < require('GameManager').getInstance().ketDataConfig.fee.length; i++) {
            if (require('GameManager').getInstance().user.vip === i) {
                this.lb_fee.string = 'fee:' + require('GameManager').getInstance().ketDataConfig.fee[i] + '%';
            }
        }
