...
cc.Class({
  ...
  startGame() {
    ...
    this.loadGameList();
    this.btn_cashOut.node.active = require("GameManager").getInstance().is_dt;
  ...
  setInfoButonWithConfig() {
    ...
    let vChanpho = require("GameManager").getInstance().vchanpho;
    this.btn_sendGift.node.active = myVip >= vChanpho ? true : false;
    this.btn_giftCode.node.active = !this.btn_giftCode.node.active//require("GameManager").getInstance().ismaqt;
  ...
  updateName() {
    ...
    if (this.lbName.node.width > 157) {
      let acMove1 = cc.moveTo(1.0, -this.lbName.node.width / 6, 0);
      let acMove2 = cc.moveTo(1.0, 0, 0);
