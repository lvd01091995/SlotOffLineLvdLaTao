var ItemInviteInTable = cc.Class({
    ...
    init (index, chip, strData) {
        if (strData) {
            ...
            this.avatar.node.getComponent("AvatarItem").loadTexture(data.idAva, data.name, data.fbid);

            // if (data.idAva > 0 && data.idAva < 999) {
            //     var url_ava = require('GameManager').getInstance().avatar_link.replace("%avaNO%", data.idAva);
