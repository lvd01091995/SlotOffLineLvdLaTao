  ...
  switch (evt) {
    case 'filter': {
      ...
      while (listDataResendForPacket.length > 0) {
        ...
        cc.log("SML Gui Lai Thang Nay:" + JSON.stringify(resend.strData));
...
var SMLSocketIO = cc.Class({
  ...
  startSIO() {
    ...
    if (url == '' || url == null) url = '';
  //  url = "http://192.168.1.170:3000/1st";//hien cmt;
    ...
    this._socketIO.on("connect", () => {
      ...
      if (this.DATAEVT0 != null) {
        ...
        cc.log("SML GUI LAI EVT 0:" + this.DATAEVT0);
  ...
  emitSIO(eventName, strData) {
    ...
    if (this._socketIO != null && this.isConnected) {
      cc.NGWlog("SIO: gui duoc emit len roi");
      this._socketIO.emit("event", strData);
    } else {
      cc.NGWlog("SIO: deo data leen");
  ...
  emitSIOWithValuePacket(packetValue, namePackage, isSend, isPacketDetai, timeStamp = 0) {
    ...
    if (!this.isConnected || !this.isGetedListFillter) {
      ...
      cc.log("SML GUi THANG NAY VAO LIST RESEND SAU:" + JSON.stringify(objSave.strData));
      return;
    }
    let evtt = "";
    ...
    cc.NGWlog("====> SIO: DATA ====>" + JSON.stringify(objectVV));
  ...
  emitReginfo() {
    ...
    cc.NGWlog("===== SIO: GET_INFO_DEVICE_SML:" + require('GameManager').getInstance().language + require('GameManager').getInstance().devicename + require('GameManager').getInstance().brand);
    cc.NGWlog("===== SIO: TRACKING EVENT CONNECT: " + JSON.stringify(objectVL));
  ...
  emitLogin() {
    ...
    cc.NGWlog("===== SIO: TRACKING EVENT CONNECT: emitLogin");
    //tracking io khi login success
    let mapDataLogin = new Map();

    cc.NGWlog('-> SIO: emitLogin 1', mapDataLogin);
    mapDataLogin.set("event", LOGIN);
    cc.NGWlog('-> SIO: emitLogin 2', mapDataLogin);
  ...
  emitUpdateInfo() {
    ...
    cc.NGWlog("my chip test of si: " + require('GameManager').getInstance().user.ag);
    // return;
